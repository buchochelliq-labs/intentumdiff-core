...
cargo build --release          # the cdylib (intentumdiff_rust_core.{dll,so,dylib}) + rlib
```
...
- **tier-c-wasm tests** (25 of 203) load staged parser components. Without them:
  ...
  `INTENTUMDIFF_TEST_WASM_DIR` to a directory of built parser `.wasm` files.
...
```bash
cd crates/cli
cargo build --release          # -> target/release/intentumdiff
./target/release/intentumdiff engine   # prints the native banner
```

The CLI resolves the bundled parser dir via `--wasm-dir`, then `$INTENTUMDIFF_WASM_DIR`, then a
...
[intentumdiff-plugin-sdk](https://github.com/buchochelliq-labs/intentumdiff-plugin-sdk) `v0.1.0`;
