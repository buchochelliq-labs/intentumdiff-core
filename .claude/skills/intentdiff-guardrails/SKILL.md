---
name: intentdiff-guardrails
description: >-
  IntentDiff's protected-config guardrails — the project policy that flags semantically risky
  changes to protected settings (API keys, immutable fields, resource identities) in config/data
  files. Use this whenever you work on guardrail detection, the `intentdiff.yaml` policy, the
  ...
  important-vs-immutable severity model, and the CLI/extension wiring. Read intentdiff-engine for
  where guardrails sit in the pipeline and intentdiff-language-profiles for the keyed/resource
  identity model guardrails rely on; new authoritative logic belongs in the Rust core.
---

# IntentDiff — Protected-config guardrails
...
Core module: `src/intentdiff/analysis/guardrails.py` (test-oracle; the authoritative engine
destination is Rust — see `intentdiff-architecture`). Deep doc: `docs/GUARDRAILS.md`.
...
(`keyed_data_key` / `resource_profile_key`, see `intentdiff-language-profiles`), not a line number
...
## The policy file (`intentdiff.yaml`)

`GUARDRAIL_POLICY_FILENAME = "intentdiff.yaml"` at the project root (or an explicit
...
`intentdiff.yaml` itself are protected by default**, so the policy can't be silently weakened.
...
- `guardrails_enabled: bool = True` — run project `intentdiff.yaml` checks.
...
- **VS Code extension:** violations become `vscode.Diagnostic`s in the Problems panel, are
  **pinned at the top** of the Semantic Changes tree and review panel, and map to **critical**
  risk in intent/release-notes (see `intentdiff-vscode` / `intentdiff-release-notes`). They are
  the highest-severity review signal — never suppressed as noise.

## Working on guardrails

- Reproduce: put an `intentdiff.yaml` beside a fixture, diff a protected-path change via the real
