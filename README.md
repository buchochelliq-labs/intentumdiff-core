...
[![CI](https://github.com/buchochelliq-labs/intentumdiff-core/actions/workflows/ci.yml/badge.svg)](https://github.com/buchochelliq-labs/intentumdiff-core/actions/workflows/ci.yml)
...
IntentumDiff diffs code by *meaning*, not lines. It parses both sides into semantic trees,
...
crates/rust-core-host/   the engine: CST -> SemanticNode conversion, tree diff, NodeFacts,
                         ...
                         exposing the stable C ABI (intentumdiff_call) + an rlib for
                         in-process Rust consumers.
crates/cli/              the native `intentumdiff` CLI (clap) — drives the engine in-process.
...
[intentumdiff-plugin-sdk](https://github.com/buchochelliq-labs/intentumdiff-plugin-sdk) and
registered in [intentumdiff-registry](https://github.com/buchochelliq-labs/intentumdiff-registry)
— the root of trust that pins every official component to a commit + SHA-256 checksums.

## The language boundary

One stable C ABI: `intentumdiff_call(name, args_json) -> json envelope` (+ `intentumdiff_free`).
...
# the CLI
cd crates/cli && cargo build --release           # -> target/release/intentumdiff
...
(`buchochelliq-labs/intentumdiff`), which remains the archive of record. The
